...
func GetLogPlayerEquipTable(ctx *context.Context) table.Table {

	logPlayerEquip := table.NewDefaultTable(table.Config{
		Driver:     "mysql",
		Connection: "db_log",
		CanAdd:     false,
		Editable:   false,
	...
	info.AddField(gotext.Get("记录类型"), "LogType", db.Int).
		FieldFilterable(types.FilterType{FormType: form.SelectSingle}).
		FieldFilterOptions(def.EquipLog{}.GetOptions()).
		FieldDisplay(func(model types.FieldModel) interface{} {
			temp, _ := strconv.Atoi(model.Value)
			return def.EquipLog{}.GetOpStrings()[temp]
		})
	logType := 999
	...
	if logType == def.Fix || strings.Index(ctx.Request.RequestURI, "/export/") != -1 {
		info.AddField(gotext.Get("修理方式"), "FixEquipType", db.Int).
			FieldDisplay(func(model types.FieldModel) interface{} {
				if model.Row["LogType"].(int) == def.Fix {
					if model.Value == "0" {
						return "npc修理"
					}
					return gotext.Get("背包修理")
				}
				return ""
			})
	}
	if logType == def.Forge || logType == def.Append || strings.Index(ctx.Request.RequestURI, "/export/") != -1 {
		//info.AddField(gotext.Get("操作前等级"), "OldLevel", db.Int)
		info.AddField(gotext.Get("操作后等级"), "AfterOpLevel", db.Int)
	}
	if logType == def.Regenerate || logType == def.Evolution || strings.Index(ctx.Request.RequestURI, "/export/") != -1 {
		info.AddField(gotext.Get("变更属性ID"), "ChangeAttrIdx", db.Int)
		info.AddField(gotext.Get("变更属性值"), "ChangeAttrVal", db.Int)
	}
	info.AddField(gotext.Get("记录时间"), "LogTime", db.Datetime).
		FieldFilterable(types.FilterType{FormType: form.DatetimeRange}).
		FieldSortable()

	info.SetTable("log_player_equip").SetTitle(gotext.Get("装备日志")).
		...
		func(param parameter.Parameters) (data []map[string]interface{}, size int) {
			...
			if strings.Index(ctx.Request.RequestURI, "/export/") != -1 && len(serverIDS) != 0 {
				for _, v := range serverIDS {
					...
					sql1, sql2 := fusion.CreateUnionTableSqlByDays(logTimeStart, logTimeEnd, def.EquipLog{}.TableName(),
						gormDB1, def.EquipLog{}, where, false)
					gormDB1.Raw(sql1).Scan(&equipLog)
					for i, _ := range equipLog {
						equipLog[i].LogTime = strings.Replace(equipLog[i].LogTime, "T", " ", -1)
						equipLog[i].LogTime = strings.Replace(equipLog[i].LogTime, "Z", " ", -1)
					}
					for _, v := range equipLog {
						m3 := structs.Map(&v)
				...
				sql1, sql2 := fusion.CreateUnionTableSqlByDays(logTimeStart, logTimeEnd, def.EquipLog{}.TableName(),
					gormDB, def.EquipLog{}, where, false)
				if !param.IsAll() {
					if param.SortField != "" {
	...
	info.SetExportProcessFn(func(param parameter.Parameters) (types.PanelInfo, error) {
		panelInfo, _ := logPlayerEquip.GetData(param.WithIsAll(param.IsAll()))
		return types.PanelInfo{Thead: panelInfo.Thead, InfoList: panelInfo.InfoList}, nil
	})
	return logPlayerEquip
}
