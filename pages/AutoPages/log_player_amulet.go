...
func GetLogPlayerAmuletTable(ctx *context.Context) table.Table {

	logPlayerAmulet := table.NewDefaultTable(
		table.Config{
			Driver:     "mysql",
			Connection: "db_log",
			CanAdd:     false,
	...
	info.AddField(gotext.Get("记录类型"), "LogType", db.Int).
		FieldFilterable(types.FilterType{FormType: form.SelectSingle}).
		FieldFilterOptions(def.AmuletLog{}.GetOptions()).
		FieldDisplay(
	...
	if logType == def.AmuletRemove {
		info.AddField(gotext.Get("艾尔特类型ID"), "ErtlTypeID", db.Mediumtext).
		...
		info.AddField(gotext.Get("艾尔特属性"), "ItemErtl", db.Mediumtext)
	}
	info.AddField(gotext.Get("记录时间"), "LogTime", db.Datetime).
		FieldFilterable(types.FilterType{FormType: form.DatetimeRange}).
		FieldSortable()

	info.SetTable("log_player_amulet").SetTitle(gotext.Get("配饰镶嵌记录")).
		...
		func(param parameter.Parameters) (data []map[string]interface{}, size int) {
			...
			if strings.Index(
				...
				for _, v := range serverIDS {
					...
					sql1, sql2 := fusion.CreateUnionTableSqlByDays(logTimeStart, logTimeEnd, def.AmuletLog{}.TableName(), gormDB1, def.AmuletLog{}, where, false)
					gormDB1.Raw(sql1).Scan(&amuletLog)
					for i, _ := range amuletLog {
						amuletLog[i].LogTime = strings.Replace(amuletLog[i].LogTime, "T", " ", -1)
						amuletLog[i].LogTime = strings.Replace(amuletLog[i].LogTime, "Z", " ", -1)
					}
					for _, v := range amuletLog {
						m3 := structs.Map(&v)
				...
				sql1, sql2 := fusion.CreateUnionTableSqlByDays(logTimeStart, logTimeEnd, def.AmuletLog{}.TableName(), gormDB, def.AmuletLog{}, where, false)
				if !param.IsAll() {
					if param.SortField != "" {
				...
				gormDB.Raw(sql1).Scan(&amuletLog)
				for i, _ := range amuletLog {
					amuletLog[i].LogTime = strings.Replace(amuletLog[i].LogTime, "T", " ", -1)
					amuletLog[i].LogTime = strings.Replace(amuletLog[i].LogTime, "Z", " ", -1)
				}
	...
	info.SetExportProcessFn(
		func(param parameter.Parameters) (types.PanelInfo, error) {
			panelInfo, _ := logPlayerAmulet.GetData(param.WithIsAll(param.IsAll()))
			return types.PanelInfo{Thead: panelInfo.Thead, InfoList: panelInfo.InfoList}, nil
		},
	)

	return logPlayerAmulet
}
