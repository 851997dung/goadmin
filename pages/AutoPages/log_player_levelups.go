...
func GetLogPlayerLevelupsTable(ctx *context.Context) table.Table {

	logPlayerLevelups := table.NewDefaultTable(table.Config{
		Driver:     "mysql",
		Connection: "db_log",
		CanAdd:     true,
		Editable:   true,
	...
	info := logPlayerLevelups.GetInfo()
	...
	info.AddField("Id", "Id", db.Bigint).
		FieldHide()
	info.AddField(gotext.Get("账号"), "AcctId", db.Int).FieldHide()
	...
	info.AddField(gotext.Get("角色等级"), "PlayerLevel", db.Int).
		...
		FieldSortable()
	info.AddField("PlayerVipLevel", "PlayerVipLevel", db.Int).
		FieldHide()
	...
	info.SetTable("log_player_levelups").SetTitle(gotext.Get("升级日志")).
	...
	info.SetGetDataFn(
		func(param parameter.Parameters) (data []map[string]interface{}, size int) {
			...
			fields := fusion.GetFields2Struct(def.LevelUpLog{})
			sql1 := "select " + fields + " from " + def.LevelUpLog{}.TableName()
			sql2 := "select count(*) from " + def.LevelUpLog{}.TableName()
			where := ""
			if acctId != "" {
			...
			if strings.Index(ctx.Request.RequestURI, "/export/") != -1 && len(serverIDS) != 0 {
				for _, v := range serverIDS {
					...
					gormDB1.Raw(sql1).Scan(&levelUpLog)
					for i, _ := range levelUpLog {
						levelUpLog[i].LogTime = strings.Replace(levelUpLog[i].LogTime, "T", " ", -1)
						levelUpLog[i].LogTime = strings.Replace(levelUpLog[i].LogTime, "Z", " ", -1)
					}
					for _, v := range levelUpLog {
						m3 := structs.Map(&v)
				...
				gormDB.Raw(sql1).Scan(&levelUpLog)
				for i, _ := range levelUpLog {
					levelUpLog[i].LogTime = strings.Replace(levelUpLog[i].LogTime, "T", " ", -1)
					levelUpLog[i].LogTime = strings.Replace(levelUpLog[i].LogTime, "Z", " ", -1)
				}
	...
	info.SetExportProcessFn(func(param parameter.Parameters) (types.PanelInfo, error) {
		panelInfo, _ := logPlayerLevelups.GetData(param.WithIsAll(param.IsAll()))
		return types.PanelInfo{Thead: panelInfo.Thead, InfoList: panelInfo.InfoList}, nil
	})
	return logPlayerLevelups
}
