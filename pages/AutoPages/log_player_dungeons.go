...
func GetLogPlayerDungeonsTable(ctx *context.Context) table.Table {

	logPlayerDungeons := table.NewDefaultTable(table.Config{
		Driver:     "mysql",
		Connection: "db_log",
		CanAdd:     false,
		Editable:   false,
	...
	info := logPlayerDungeons.GetInfo()
	...
	info.AddField("PlayerRichValue", "PlayerRichValue", db.Double).
		FieldHide()
	info.AddField(gotext.Get("副本名称"), "DungeonId", db.Int).
		FieldFilterable().FieldDisplay(func(model types.FieldModel) interface{} {
		temp, _ := strconv.Atoi(model.Value)
		var ops = def.DungeonsLog{}.GetOpStrings()
		return ops[temp]
	})
	info.AddField(gotext.Get("副本进度"), "Progress", db.Int).
		FieldFilterable().FieldDisplay(func(model types.FieldModel) interface{} {
		temp, _ := strconv.ParseBool(model.Value)
		if !temp {
			return gotext.Get("进入")
		}
	...
	info.SetTable("log_player_dungeons").SetTitle(gotext.Get("副本日志")).
	...
	info.SetGetDataFn(
		func(param parameter.Parameters) (data []map[string]interface{}, size int) {
			...
			dungeonId := ctx.Request.FormValue("DungeonId")
			isClear := ctx.Request.FormValue("IsClear")

			fields := fusion.GetFields2Struct(def.DungeonsLog{})
			sql1 := "select " + fields + " from " + def.DungeonsLog{}.TableName()
			sql2 := "select count(*) from " + def.DungeonsLog{}.TableName()
			where := ""
			if acctId != "" {
			...
			if dungeonId != "" {
				where += " dungeonId = " + dungeonId + " and "
			}
			if isClear != "" {
				where += " progress = " + isClear + " and "
			}
			if logTimeStart != "" {
			...
			if strings.Index(ctx.Request.RequestURI, "/export/") != -1 && len(serverIDS) != 0 {
				for _, v := range serverIDS {
					...
					gormDB1.Raw(sql1).Scan(&dungeonsLog)
					for i, _ := range dungeonsLog {
						dungeonsLog[i].LogTime = strings.Replace(dungeonsLog[i].LogTime, "T", " ", -1)
						dungeonsLog[i].LogTime = strings.Replace(dungeonsLog[i].LogTime, "Z", " ", -1)
					}
					for _, v := range dungeonsLog {
						m3 := structs.Map(&v)
				...
				gormDB.Raw(sql1).Scan(&dungeonsLog)
				for i, _ := range dungeonsLog {
					dungeonsLog[i].LogTime = strings.Replace(dungeonsLog[i].LogTime, "T", " ", -1)
					dungeonsLog[i].LogTime = strings.Replace(dungeonsLog[i].LogTime, "Z", " ", -1)
				}
	...
	info.SetExportProcessFn(func(param parameter.Parameters) (types.PanelInfo, error) {
		panelInfo, _ := logPlayerDungeons.GetData(param.WithIsAll(param.IsAll()))
		return types.PanelInfo{Thead: panelInfo.Thead, InfoList: panelInfo.InfoList}, nil
	})
	return logPlayerDungeons
}
